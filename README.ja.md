...
## 可変的なエージェンティックレベル (Variable Agentic Levels)

このフレームワークの強みの一つは、「エージェンティックレベル（自律性の度合い）」を自由に調整できる点です。

### 1. 決定論的ワークフロー (Low Agentic)
オーケストレーターを決定論的なコードとして実装します。状態遷移はLLMの判断ではなく、固定されたルールによって定義されます。
- **ユースケース**: 信頼性と再現性が重要なタスク。
- **オーケストレーション**: `if score > 80: fsm.transition("approve")`
- **LLMの役割**: 特定のサブタスク（要約、抽出など）にのみ使用。

### 2. ハイブリッドエージェント (The "Middle")
標準的なユースケースです。FSMが「地図（境界線）」を定義し、LLMがその中での経路を決定します。
- **ユースケース**: 一般的なアシスタントボット、複雑なデータ処理。
- **オーケストレーション**: `fsm.get_next_states()` からLLMが次の状態を選択。
- **LLMの役割**: ツールの使用と状態遷移の両方を決定。

### 3. フルエージェンティック (High Agentic)
ゴールとツールを与えられたエージェントが自由に探索します。FSMは高レベルのフェーズ管理（例：計画 -> 実行 -> レビュー）にのみ使用されます。
- **ユースケース**: リサーチ、オープンエンドな問題解決。
- **オーケストレーション**: 複数の遷移肢があり、エージェントが最適なパスを戦略的に決定。
- **LLMの役割**: FSMで定義された大枠のフェーズ内で完全に自律的に動作。
