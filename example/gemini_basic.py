...
def main():
    ...
    # Shared Execution Context (Closure-based)
    execution_context = {}
    ...
    def write_article(topic: str, research_summary: str) -> str:
        """Writes a short article based on research and stores it in context."""
        print(f"\n[Tool] Writing article on {topic}...")
        content = f"Title: {topic}\n\nSummary: {research_summary}\n\n(Article content... generated by AI)"

        # 1. Save to Shared Context
        execution_context["current_article"] = content

        # 2. Save artifact to disk
        filename = f"{topic.replace(' ', '_').lower()}_article.txt"
        with open(filename, "w") as f:
            f.write(content)
        print(f"[Tool] Artifact saved to {filename}")

        # 3. Return summary to LLM (Token Optimization)
        return f"Article on '{topic}' has been written and stored in shared context. Length: {len(content)} characters."

    @tools.register
    def review_article() -> str:
        """Reviews the article currently stored in the shared context. No arguments needed."""
        print(f"\n[Tool] Reviewing article from context...")

        article_content = execution_context.get("current_article")
        if not article_content:
            return "ERROR: No article found in context to review."

        print(
            f"[Tool] Read from context (length={len(article_content)}): {article_content[:50]}..."
        )
